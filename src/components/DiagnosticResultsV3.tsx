...
interface DiagnosticResultsV3Props {
  session: DiagnosticSession;
  onStartNew: () => void;
}

export const DiagnosticResultsV3: React.FC<DiagnosticResultsV3Props> = ({ session, onStartNew }) => {
  const diagnosis = session.finalDiagnosis?.finalDiagnosis;
  const source = session.finalDiagnosis?.source;
  ...
  const getSourceIcon = (source: string) => {
    switch (source) {
      case 'gemini_search': return <Brain className="w-4 h-4 text-purple-600" />;
      ...
      case 'emergency_fallback': return <AlertTriangle className="w-4 h-4 text-red-600" />;
  ...
  const getSourceDescription = (source: string) => {
    switch (source) {
      case 'gemini_search': return 'Gemini AI processed web search results for comprehensive analysis';
      case 'chatgpt_fallback': return 'ChatGPT analyzed available information as fallback solution';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Problem Diagnosis v3</h1>
