...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `You are an expert electronics repair technician. Analyze this ${deviceCategory} image carefully and identify possible problems.

IMPORTANT: Return ONLY a valid JSON object with this exact structure:
{
  "possibleProblems": ["Specific problem 1", "Specific problem 2", "Specific problem 3"],
  ...
  "clarifyingQuestions": [
    ...
    "Short specific question 6?"
  ]
}

Requirements:
- Identify 1-3 most likely specific problems based on what you can see
- Generate at least 5-6 short, specific clarifying questions
- Questions should help confirm or refine the identified problems
- Keep questions simple and answerable by a non-technical user
- Focus on observable symptoms and behaviors`;
    ...
    if (!analysis.possibleProblems || !Array.isArray(analysis.possibleProblems) || analysis.possibleProblems.length === 0) {
      analysis.possibleProblems = ["Hardware component failure", "Power supply issue", "Connection problem"];
    }
    ...
    const fallbackAnalysis = {
      possibleProblems: ["Hardware malfunction", "Component failure", "Power or connection issue"],
      visualObservations: "Unable to fully analyze the image, but I can still help with diagnosis.",
      confidence: "low",
      clarifyingQuestions: [
