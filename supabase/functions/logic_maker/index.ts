    ...
    const questionResponse = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [
            {
              text: `Based on the following analysis, generate 3 follow-up questions a user might have. Respond with a JSON array of strings, for example: ["Question 1", "Question 2"] in ${getLanguageName(language)}.
