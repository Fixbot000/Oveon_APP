-- Add daily_scan_limit to profiles table
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS daily_scan_limit integer NOT NULL DEFAULT 3;
...
BEGIN
  ...
  SELECT ispremium, timezone, scan_count, scan_count_date, daily_scan_limit
    INTO v_ispremium, v_timezone, v_scan_count, v_scan_count_date, v_daily_scan_limit
...
BEGIN
  -- Lock the user's profile row to avoid race conditions
  SELECT ispremium, daily_scan_limit, scan_count
    INTO v_ispremium, v_daily_scan_limit, v_scan_count
  FROM public.profiles
  WHERE id = p_user_id
  FOR UPDATE;
  ...
  -- Increment the daily scan limit
